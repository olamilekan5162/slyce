...
const Dashboard = () => {
  ...
  const totalEarned = recipientSplits.reduce((acc, curr) => acc + curr.totalReceived, 0);
  ...
  return (
    <div className={`container ${styles.dashboard}`}>
      ...
      <div className={styles.tabContent}>
        {activeTab === 'active' && <SplitTable splits={activeSplits} type="active" />}
        {activeTab === 'closed' && <SplitTable splits={closedSplits} type="closed" />}
        {activeTab === 'recipient' && (
          <>
            <SplitTable splits={recipientSplits} type="recipient" />
            <div className={styles.summaryLine}>
              Total earned via Slyce: <span className="monospace-numbers">${totalEarned.toFixed(2)}</span>
            </div>
          </>
        )}
      </div>
